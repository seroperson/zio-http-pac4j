		...
		{#if data.profile}
			<div class="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
				<Avatar class="size-8 rounded-lg">
					<AvatarImage src={data.profile.avatar} alt={data.profile.username} />
					<AvatarFallback class="rounded-lg">CN</AvatarFallback>
				</Avatar>
				<div class="grid flex-1 text-left text-sm leading-tight">
					<span class="truncate font-medium">{data.profile.username}</span>
					<span class="truncate text-xs">{data.profile.email}</span>
				</div>
				<Button
					variant="outline"
					class={cn('w-full')}
